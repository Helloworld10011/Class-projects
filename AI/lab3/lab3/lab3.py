...
quick_to_win_player = lambda board: minimax(board, depth=6,
...
def alpha_beta_find_board_value(board, depth, eval_fn,
                             ...
                             beta= INFINITY, alpha=NEG_INFINITY):
    """
...
def alpha_beta_search(board, depth, eval_fn, get_next_moves_fn=get_all_next_moves, is_terminal_fn=is_terminal, verbose= True):
    alpha= NEG_INFINITY
    beta= INFINITY
...
#    first idea: we don't just weigh the longest chain, but the all of chains with two or more nodes.(*5)
#    second idea: if we have multiple triplet chains thorough a node, we double the score of weights.(*10)
#    third: having tokens in the center of the board is still good.
#    fourth:
    if board.is_game_over():
        score = -2000 + board.num_tokens_on_board()
    else:
        ...
        for row in range(6):
            for col in range(7):
                if board.get_cell(row, col) == me:
                    set_1.update(filter(lambda x: len(x) > 2, board._chain_sets_from_cell(row, col)))
                elif board.get_cell(row, col) == other:
                    set_2.update(filter(lambda x: len(x) > 2, board._chain_sets_from_cell(row, col)))

        set_3= set(filter(lambda x: len(x) > 2, board.chain_cells(me)))
        set_4= set(filter(lambda x: len(x) > 2, board.chain_cells(other)))

        set_3= set_3- set_1
        set_4= set_4- set_2

        score= 10*(len(set_1)-len(set_2))+5*(len(set_3)-len(set_4))
...
my_player= lambda board: alpha_beta_search(board,
                                           depth=6,
...
if False:
    ...
    test_board= ConnectFourBoard(board_array= board_tuples, current_player= 1)
    print(better_evaluate(test_board))
