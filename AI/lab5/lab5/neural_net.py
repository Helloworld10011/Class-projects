...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        for i in range(len(self.my_weights)):
            if isinstance(self.my_inputs[i], Input): value= self.my_inputs[i].get_value()
            else: value= self.my_inputs[i].output()
            out+= value*self.my_weights[i].get_value()
        return sigmoid(out)
    ...
    def compute_doutdx(self, elem):
        ...
        if elem in self.my_weights:
            ...
            if isinstance(self.my_inputs[index], Input): x= self.my_inputs[index].get_value()
            else: x= self.my_inputs[index].output()
            return out*(1- out)*x
        else:
            D= 0
            for i in range(len(self.my_weights)):
                if isinstance(self.my_inputs[i], Neuron):
                    D+= out*(1- out)*self.my_weights[i].get_value()*self.my_inputs[i].compute_doutdx(elem)
            return D
...
def sigmoid(x):
    ...
    """
    pos_mask = (x >= 0)
    z = math.exp(-x) * pos_mask+ math.exp(x) * (1 - pos_mask)
    top = 1 * pos_mask + z*(1 - pos_mask)
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        if isinstance(self.my_input, Input):
            P= -0.5*(self.my_desired_val- self.my_input.get_value())**2
            return P
        else:
    ...
    def dOutdX(self, elem):
        ...
        if isinstance(self.my_input, Input):
            ...
            out= self.my_input.output()
            return out*(1- out)*(self.my_desired_val - self.my_input.output())*self.my_input.dOutdX(elem)
...
def random_weight():
    ...
    return float(np.random.randn())/math.sqrt(2)
...
def abs_mean(values):
    ...
    abs_vals = list(map(lambda x: abs(x), values))
    total = sum(abs_vals)
