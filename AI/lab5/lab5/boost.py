...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        for classifier, alpha in self.classifiers:
            score+= classifier.classify(obj)*alpha
        if score>=0: return 1
        else: return -1
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        for datum, weight in zip(self.data, self.data_weights):
            weight*= math.exp(-alpha* best_classifier.classify(datum)* self.standard.classify(datum))
