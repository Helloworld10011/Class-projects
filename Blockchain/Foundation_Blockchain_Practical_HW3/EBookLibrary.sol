...
contract EBookLibrary is CustomERC20{
     ...
     */
    constructor(uint _accessDuration , uint _premiumFee) CustomERC20(100) public{
    ...
    public{
        ...
        rentedBooks[bookId].renter= admin;
    ...
    public{
        ...
        if(rentedBooks[bookId].available == true) {
            amount= books[bookId].accessFee;
            _burn(msg.sender, amount);
            rentedBooks[bookId].available= false;
            rentedBooks[bookId].renter= msg.sender;
            rentedBooks[bookId].startTime= now;
          } else{
            require(accoutTypes[msg.sender] == AccountType.premium);
            require(accoutTypes[rentedBooks[bookId].renter] == AccountType.regular);
            amount= books[bookId].accessFee;
            _burn(msg.sender, amount);
            _mint(rentedBooks[bookId].renter, amount);
            rentedBooks[bookId].renter= msg.sender;
            rentedBooks[bookId].startTime= now;
          }
    }
