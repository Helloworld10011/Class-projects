...
class ACAgent(BaseAgent):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        Values_n= self.critic.forward(ob_no)
        Values_next_n= self.critic.forward(next_ob_no)

        adv_n = re_n+ (1-terminal_n)* self.gamma*(Values_next_n)- Values_n
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for _ in range(self.agent_params['num_critic_updates_per_agent_update']):
            loss_critic= self.critic.update(ob_no, next_ob_no, re_n, terminal_n)
        ...
        loss = OrderedDict()
        loss['Critic_Loss'] = loss_critic  # put final critic loss here
        loss['Actor_Loss'] = loss_actor  # put final actor loss here
        return loss
