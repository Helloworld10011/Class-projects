...
def lander_model(obs, num_actions, scope, reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("action_value"):
            out = layers.dense(out, units=64, activation='relu')
            out = layers.dense(out, units=64, activation='relu')
...
def atari_model(img_input, num_actions, scope, reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("convnet"):
            ...
            out = layers.conv2d(out, filters=32, kernel_size=8, strides=4, activation= 'relu')
            out = layers.conv2d(out, filters=64, kernel_size=4, strides=2, activation= 'relu')
            out = layers.conv2d(out, filters=64, kernel_size=3, strides=1, activation= 'relu')
        out = layers.flatten(out)
        with tf.variable_scope("action_value"):
            out = layers.dense(out, units=512, activation= 'relu')
            out = layers.dense(out, units=num_actions)
