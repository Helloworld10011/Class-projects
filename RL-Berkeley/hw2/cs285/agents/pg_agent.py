...
class PGAgent(BaseAgent):
    def __init__(self, sess, env, agent_params):
        ...
        self.lambd= self.agent_params['GAE']
    ...
    def estimate_advantage(self, obs, q_values, rew_list):
        ...
        if self.nn_baseline:
            if self.lambd==0.0:
                b_n_unnormalized = self.actor.run_baseline_prediction(obs)[0]
                b_n = b_n_unnormalized * np.std(q_values) + np.mean(q_values)
                adv_n = q_values-b_n
            else:
                l= 2*math.ceil(1/(1-self.gamma*self.lambd))
                rews= self.paste_rewards(rew_list)
                pass
    ...
    def _discounted_return(self, rewards):
        ...
        list_of_discounted_returns= np.sum(gammas*rewards)*np.ones_like(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)):
            gammas = self.gamma ** (np.arange(len(rewards)- start_time_index))
            sum_discounted_rtg = np.sum(gammas * rewards[start_time_index:])
    ...
    def paste_rewards(self, rew_list):
        rewards= []
        for rew in rew_list:
            rewards.extend(rew)
