...
class MLPPolicy(BasePolicy):
    ...
    def __init__(self, sess, ac_dim, ob_dim, n_layers, size, learning_rate=1e-4, training=True,
                 policy_scope='policy_vars', discrete=False, nn_baseline=False, **kwargs):
    ...
    def define_placeholders(self):
        self.observations_pl= tf.placeholder(dtype= tf.float32, shape=[None, self.ob_dim], name= "obs")
        self.actions_pl= tf.placeholder(dtype=tf.int32, shape=[None, 1], name= "acts")

    def define_forward_pass(self):
        # TODO implement this build_mlp function in tf_utils
        mean = build_mlp(self.observations_pl, output_size=self.ac_dim, scope='continuous_logits',
    ...
    def build_action_sampling(self):
        ...
        self.sample_ac = mean + tf.exp(logstd) * tf.random.normal(tf.shape(mean), 0, 1)

    def define_train_op(self):
        self.loss= tf.losses.mean_squared_error(self.actions_pl, self.sample_ac)
    ...
    def get_action(self, obs):
        ...
        act= self.sess.run([self.sample_ac], feed_dict= {self.observations_pl: obs})
    ...
    def update(self, observations, actions):
        ...
        self.sess.run([self.updates], feed_dict= {self.observations_pl: observations, self.actions_pl: actions})
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(self, observations, actions):
        assert(self.training, 'Policy must be created with training=True in order to perform training updates...')
        l, _= self.sess.run([self.loss, self.train_op], feed_dict={self.observations_pl: observations, self.acs_labels_na: actions})
